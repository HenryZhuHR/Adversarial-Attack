...
if __name__ == '__main__':
    ...
    for images, labels in pbar:
        ...
        if step<=1:
            ...
            if not os.path.exists('images/pgd'): os.makedirs('images/pgd')
            from attack.utils import save_image_tensor
            save_image_tensor('images/pgd/e=%d~a=%d~s=%d-src.png'%(int(EPSILON*255),int(ALPHA*255),NUM_STEPS),images[0:show_img])
            save_image_tensor('images/pgd/e=%d~a=%d~s=%d-adv.png'%(int(EPSILON*255),int(ALPHA*255),NUM_STEPS),images_adv[0:show_img])
            save_image_tensor('images/pgd/e=%d~a=%d~s=%d-per.png'%(int(EPSILON*255),int(ALPHA*255),NUM_STEPS),images_adv[0:show_img]-images[0:show_img])
    ...
    if not os.path.exists(os.path.join(LOGS,'pgd.txt')):
        with open(os.path.join(LOGS,'pgd.txt'),'a') as f:        
            f.write('(e/255)epsilon\t(a/255)alpha\ts\tnoAt acc\tattack acc\tnoAt loss\tattack loss\n')
    with open(os.path.join(LOGS,'pgd.txt'),'a') as f:       
